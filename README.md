...
### NOTE: This notebook was modified for a Windows 10 environment with python3.9+ and CUDA 11.3+ installed.
- Look around in the notebook ([![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/zenforic/jukebox-win-local/blob/master/ZenZags_Main_4_54.ipynb)) and change Shadow to your username on windows and/or change the path(s) you want to use.
- Install Microsoft MPI **IF** not using conda: https://www.microsoft.com/en-us/download/details.aspx?id=100593 or get an MPI import error (download and install both files).
- Install pytorch with cuda support and cuda 11.3+ installed IF you are using a conda environment: `conda install pytorch=1.12.1 torchvision=0.13.1 torchaudio=0.12.1 cudatoolkit=11.3 -c pytorch` OR `pip install torch=1.12.1 torchvision=0.13.1 torchaudio=0.12.1 --extra-index-url https://download.pytorch.org/whl/cu113` if running a pyenv or global. **NEW:** you can now try: `conda env create -f .\environment.yaml` and it should perform most steps.
